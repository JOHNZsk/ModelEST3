...
begin
  case p_hodnota of
    ...
    NPT_RESETNAV: Result:='Reset návestidla';
    NPT_RESETVYH: Result:='Reset výhybky';
    NPT_RESETNAVGLOBAL,NPT_RESETNAVDOP: Result:='Reset všetkých návestidiel';
    NPT_RESETVYHGLOBAL,NPT_RESETVYHDOP: Result:='Reset všetkých výhybiek';
    NPT_PRIVOLAVACKA: Result:='Rozsvietenie priv. návesti';
    NPT_ZAV2: Result:='Zrušenie ruč. záveru';
    NPT_DOH1: Result:='Vyradenie dohľadových obvodov výhybky';
    NPT_DOH2: Result:='Zrušenie vyradenia dohľadu výhybky';
    ...
    NPT_KSV: Result:='Kontrola štítkov a výluk';
    else Result:='Núdzový povel';
  end;
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          DiagDlg.Memo1.Lines.Insert(0,'Duplicitní cesta: '+IntToStr(cesta.Cislo));
          Result:=False;
        end;
      end;
    finally
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        VytvorPoruchu(Now,t_stavana_vyhybka.Dopravna,'Nie sú splnené podmienky pre prestavenie');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else VytvorPoruchu(Now,zdroj.Dopravna,'Cestu nejde navoliť');
...
begin
  ...
  else VytvorPoruchu(Now,p_navestidlo.Dopravna,'Nejde rozsvietiť ďalšiu PN na zhlaví');
end;
...
begin
  p_popis_a:='(A) Základná';
  p_popis_b:='(B) Opačná';
  p_popis_c:='(C) Základná nepotvrdená';
  p_popis_d:='(D) Opačná nepotvrdená';
  p_popis_e:='(E) Neznáma/bez dohľadu';
  p_text_a:='';
  p_text_b:='';
  p_text_c:='';
  p_text_d:='';
...
var
  prvok: TStavadloObjekt;
begin
  p_popis_a:='(A) Bez záveru';
  p_popis_b:='(B) Ručný záver';
  p_popis_c:='(C) Záver VC';
  p_text_a:='';
  p_text_b:='';
  p_text_c:='';
  p_cervena_a:=False;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if t_je_stitok then VytvorPoruchu(Now,cesta.Key.Dopravna,'Cestu nejde navoliť - štítok');
          if t_je_vyluka then VytvorPoruchu(Now,cesta.Key.Dopravna,'Cestu nejde navoliť - výluka');
